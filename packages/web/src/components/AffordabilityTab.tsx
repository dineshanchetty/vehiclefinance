...
export function AffordabilityTab({ deal }: { deal: DealWithRelations }) {
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        ...
        {statements.length === 0 ? (
          <div className="p-8 text-center text-sm text-gray-400">
            ...
            <div className="mt-2">
              <Link to={`/deals/${deal.id}`} className="text-indigo-600 hover:underline">
                View Documents
              </Link>
            </div>
          </div>
  ...
  const overridePane = (
    <div className="space-y-6">
      ...
      {/* ── Submit for credit ────────────────────────────────────────────── */}
      <div className="rounded-xl border border-indigo-200 bg-indigo-50/40 p-5 flex items-center justify-between gap-4">
        <div>
          <h3 className="text-sm font-semibold text-indigo-900">Ready for credit decision?</h3>
  ...
  return (
    <div className="rounded-xl border border-indigo-200 bg-indigo-50/30 p-5">
      <div className="flex items-baseline justify-between mb-3">
        <h3 className="text-sm font-semibold text-indigo-900">{months}-Month Consolidated</h3>
      ...
      {totalFeeCount > 0 && (
        <p className="mt-3 text-[11px] text-indigo-700/70">
